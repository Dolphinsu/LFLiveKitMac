...
+ (NSArray *)availableCameraDevices
{
...
- (nullable instancetype)initWithAudioConfiguration:(nullable LFLiveAudioConfiguration *)audioConfiguration videoConfiguration:(nullable LFLiveVideoConfiguration *)videoConfiguration captureType:(LFLiveCaptureTypeMask)captureType
{
	...
	return [self initWithAudioConfiguration:audioConfiguration audioDevice:audioDevice videoConfiguration:videoConfiguration videoDevice:videoDevice captureType:captureType];
}

- (nullable instancetype)initWithAudioConfiguration:(nullable LFLiveAudioConfiguration *)audioConfiguration audioDevice:(nullable AVCaptureDevice *)audioDevice videoConfiguration:(nullable LFLiveVideoConfiguration *)videoConfiguration videoDevice:(nullable AVCaptureDevice *)videoDevice captureType:(LFLiveCaptureTypeMask)captureType
{
...
{
	_videoDevice = videoDevice;
	[self.videoCaptureSource setVideoCaptureDevice:_videoDevice];
}
...
- (LFVideoCapture *)videoCaptureSource
{
	if (!_videoCaptureSource) {
		if (self.captureType & LFLiveCaptureMaskVideo) {
			_videoCaptureSource = [[LFVideoCapture alloc] initWithVideoConfiguration:_videoConfiguration videoCaptureDevice:_videoDevice];
...
- (BOOL)AVAlignment
{
	if ((self.captureType & LFLiveCaptureMaskAudio || self.captureType & LFLiveInputMaskAudio) && (self.captureType & LFLiveCaptureMaskVideo || self.captureType & LFLiveInputMaskVideo)) {
		return self.hasCaptureAudio && self.hasKeyFrameVideo;
	}
	return YES;
}
