...
{
	LFLiveCaptureMaskAudio = (1
			<< LFLiveCaptureAudio),                                 ///< only inner capture audio (no video)
	LFLiveCaptureMaskVideo = (1
			<< LFLiveCaptureVideo),                                 ///< only inner capture video (no audio)
	LFLiveInputMaskAudio = (1
			<< LFLiveInputAudio),                                     ///< only outer input audio (no video)
	LFLiveInputMaskVideo = (1
			<< LFLiveInputVideo),                                     ///< only outer input video (no audio)
...
+ (nonnull NSArray <AVCaptureDevice *> *)availableAudioDevices;
+ (nonnull NSArray <AVCaptureDevice *> *)availableCameraDevices;
...
@property (nonatomic, strong, null_resettable) NSView *preView;
 ...
 *.*/
//@property (nonatomic, strong, nullable) NSView *warterMarkView;

///* The currentImage is videoCapture shot */
//@property (nonatomic, strong, readonly, nullable) NSImage *currentImage;
...
- (nullable instancetype)initWithAudioConfiguration:(nullable LFLiveAudioConfiguration *)audioConfiguration audioDevice:(nullable AVCaptureDevice *)audioDevice videoConfiguration:(nullable LFLiveVideoConfiguration *)videoConfiguration videoDevice:(nullable AVCaptureDevice *)videoDevice captureType:(LFLiveCaptureTypeMask)captureType NS_DESIGNATED_INITIALIZER;
...
@property (nonnull, nonatomic, strong) AVCaptureDevice *audioDevice;
@property (nonnull, nonatomic, strong) AVCaptureDevice *videoDevice;
